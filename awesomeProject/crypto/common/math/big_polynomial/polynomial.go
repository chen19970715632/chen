...
type PolynomialClient struct {
	// A big prime which is used for Galois Field computing  用于伽罗瓦场计算的大素数
	prime *big.Int
}

// New new PolynomialClient with a prime  具有素数的新多项式客户端
...
// RandomGenerate make a random polynomials F(x) of Degree [degree], and the const(X-Intercept) is [intercept]  做一个次数 [度] 的随机多项式 F（x），常量（X-截距） 为 [截距]
