...
// ConfusionMatrix is a nested map of actual and predicted class counts  是实际类计数和预测类计数的嵌套映射
...
//从一组真实的类值（'realClasses'）构建一个混淆矩阵
//和一组预测的类值（“predClasses”）。
//realClasses 和 predClasses 的相同索引必须引用相同的样本
...
// String returns a human-readable version of the ConfusionMatrix. 返回人类可读的混淆矩阵版本。
...
func GetROC(realClasses []string, predValues []float64, class string) ([][3]float64, error) {
	...
	// Use the predicted value as the threshold to calculate each indicator 使用预测值作为阈值来计算每个指标
	...
	//ret[0] 表示没有实例被预测为正。
	//	FPR 和 TPR 为零，阈值任意设置为“max（predValue）+1”
	ret = append(ret, [3]float64{0, 0, stepThresholds[0] + 1})

	// calculate each point of the roc, according to the threshold array 根据阈值数组计算 ROC 的每个点
...
// GetCoordinates get the abscissa and ordinate of the point represented as [Xi,Yi,Tag], return [[Xi,Yi],...] 获取表示为 [Xi，Yi，Tag] 的点的横坐标和纵坐标，返回 [[Xi，Yi],...]
