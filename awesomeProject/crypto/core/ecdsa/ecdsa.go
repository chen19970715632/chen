...
/*
	Generates the commonly used ecdsa account, with which to identify users and nodes. 生成常用的 ecdsa 帐户，用于标识用户和节点。
*/
...
// GenerateKeyPair generate a key pair 生成密钥对
...
// ParsePrivateKey parse from local type to EC private key 从本地类型解析为 EC 私钥
...
// ParsePublicKey parse from local type to EC public key 从本地类型解析为 EC 公钥
...
// MarshalPrivateKey marshal private key to local types 将私钥封送到本地类型
...
// MarshalPublicKey marshal public key to local types 将公钥封送到本地类型
...
// DecodePrivateKeyFromString decode ec private key from string 从字符串解码 EC 私钥
...
// DecodePublicKeyFromString decode ec public key from string 从字符串解码 EC 公钥
...
// DecodeSignatureFromString decode ec signature from string 从字符串解码 EC 签名
